...
NSURL *  getThumbnailUrlWithPath(NSString *path, CGSize size)
{
    if (path == nil) {
        return nil;
    }
    
//    http://60.195.251.71/tsingda/M00/00/00/ooYBAFQar5OAZ1LDAAF4kWM0mB4244.jpg?w=100&h=100
    
    if ([path hasPrefix:@"http:"]) {
        return [NSURL URLWithString:[NSString stringWithFormat:@"%@?w=%d&h=%d", path, (int)size.width, (int)size.height]];
    }
...
//    if ([path hasPrefix:@"http:"]) {
//        
//        NSURL    * old        = [NSURL URLWithString:path];
//        NSArray  * components = [old pathComponents];
...
//            ext  = [fileName substringFromIndex:range.location];
//        }
//        
//        NSString * nfn = [NSString stringWithFormat:@"%@_%dx%d%@", name, (NSInteger)size.width, (NSInteger)size.height, ext];
//        
//        NSMutableString * xImagePath = [NSMutableString stringWithFormat:@"http://%@", old.host];
//        
//        for (int i = 1; i < [components count] - 1; i++) {
//            [xImagePath appendFormat:@"/%@", [components objectAtIndex:i]];
//        }
//        
//        [xImagePath appendFormat:@"/%@", nfn];
//        
//        return [NSURL URLWithString:xImagePath];
//    } else {
//        
//        return [NSURL fileURLWithPath:path];
//    }
...
UIColor * hexColor(int hex, float a)
{
    return [UIColor colorWithRed:((Byte)(hex >> 16))/255.0
                           green:((Byte)(hex >> 8))/255.0
                            blue:((Byte)hex)/255.0 alpha:a];
}
...
+ (id)share
{
    static Console * _ndebug = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _ndebug = [[Console alloc] init];
    });
    return _ndebug;
}
